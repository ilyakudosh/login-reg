...
import { logoutHandler } from "../feature/Auth/Signin/actions";
...
export const initAuthInterceptor = (store, errorAction) => {
  ...
  axiosInstace.interceptors.response.use(null, async error => {
    ...
    const blockedHttpCode = 403;

    const originalRequest = error.config;

    if (error.response.status === blockedHttpCode) {
      await store.dispatch(logoutHandler());
      window.location.replace("/blocked");
    }

    if (error.response.status === notFoundHttpCode) {
      await store.dispatch(logoutHandler());
    }
